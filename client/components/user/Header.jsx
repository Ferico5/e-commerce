    ...
    <ResponsiveContainer className="flex justify-between pt-5 pb-4.5">
        ...
        <nav>
          <ul className="flex gap-5">
            <li className="relative">
              <Link
                to="/"
                className={`pb-1 ${location.pathname === "/" ? 'after:content-[""] after:absolute after:left-1/2 after:translate-x-[-50%] after:bottom-[-3px] after:w-1/2 after:h-[1px] after:bg-[#323232]' : ""}`}
              >
                HOME
              </Link>
            </li>
            ...
            <li className="relative">
              <Link
                to="/about"
                className={`pb-1 ${location.pathname === "/about" ? 'after:content-[""] after:absolute after:left-1/2 after:translate-x-[-50%] after:bottom-[-3px] after:w-1/2 after:h-[1px] after:bg-[#323232]' : ""}`}
              >
                ABOUT
              </Link>
            </li>
            <li className="relative">
              <Link
                to="/contact"
                className={`pb-1 ${location.pathname === "/contact" ? 'after:content-[""] after:absolute after:left-1/2 after:translate-x-[-50%] after:bottom-[-3px] after:w-1/2 after:h-[1px] after:bg-[#323232]' : ""}`}
              >
                CONTACT
              </Link>
            </li>
            <li className="relative">
              <Link
                to="/admin/login"
                className="border rounded-full border-[#E5E7EB] px-5 py-2 text-[.75em] font-semibold"
              >
      ...
      <div className="flex items-center gap-6 sm:gap-4">
            ...
            <div className="absolute right-0 mt-3 w-40 bg-[#F1F5F9] rounded shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-10">
              <ul className="text-[#6B7280] font-semibold font-outfit">
                <li>
                  <Link
                    to="/orders"
                    className="block px-4 py-2 hover:text-[#000]"
                  >
                    Orders
                  </Link>
                </li>
        ...
        <a href="/cart" className="relative inline-block">
          ...
          <span className="absolute -bottom-1 -right-1 bg-black text-white p-2 text-[9px] w-[14px] h-[14px] rounded-full flex items-center justify-center font-outfit">
            {cartCount}
          </span>
        </a>
  ...
  ${open ? "translate-x-0" : "translate-x-full"}`}
      >
        ...
        <div className="flex flex-col items-start text-sm">
          <Link
            to="/"
            className={`border-b border-t border-gray-200 w-full py-1 px-2 ${location.pathname === "/" ? "bg-black text-white" : ""}`}
            onClick={() => setOpen(false)}
          >
            HOME
          </Link>
          <Link
            to="/collection"
            className={`border-b border-t border-gray-200 w-full py-1 px-2 ${location.pathname === "/collection" ? "bg-black text-white" : ""}`}
            onClick={() => setOpen(false)}
          >
            COLLECTION
          </Link>
          <Link
            to="/about"
            className={`border-b border-t border-gray-200 w-full py-1 px-2 ${location.pathname === "/about" ? "bg-black text-white" : ""}`}
            onClick={() => setOpen(false)}
          >
            ABOUT
          </Link>
          ...
          {token ? (
            <>
              <Link
                to="/orders"
                className={`border-b border-t border-gray-200 w-full py-1 px-2 ${location.pathname === "/orders" ? "bg-black text-white" : ""}`}
                onClick={() => setOpen(false)}
              >
                Orders
              </Link>
