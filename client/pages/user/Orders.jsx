    ...
    <ResponsiveContainer className="flex flex-col items-start pt-12 border-t border-[#E5E7EB]">
      ...
      {orders.length === 0 ? (
        ...
        orders.map((order) => (
          <div
            key={order._id}
            className="flex w-full justify-between border-b border-t border-[#E5E7EB] py-5"
          >
            <div className="w-full flex mr-10">
              <div className="relative w-[88px] h-[88px] mr-3">
                <img
                  ...
                  alt="Product Image"
                  loading="lazy"
                  className="w-22 object-cover rounded border border-gray-200 z-10 relative"
                />
                {order.items.length > 1 && (
                  <img
                    ...
                    alt="Product Image"
                    loading="lazy"
                    className="w-22 object-cover rounded border border-gray-200 absolute top-2 left-2 z-0 opacity-80"
                  />
                )}
              </div>
              <div className="ml-5">
            ...
            <div className="w-3/5 flex items-center justify-end">
              <Link
                to={`/orders/${order._id}`}
                className="border border-[#E5E7EB] px-4 py-2 text-sm hover:cursor-pointer"
              >
                Detail Order
              </Link>
            </div>
