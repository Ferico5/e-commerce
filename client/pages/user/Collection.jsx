...
const Collection = () => {
  ...
  const [showSearch, setShowSearch] = useState(
    location.state?.showSearch || false,
  );
  ...
  const getFilteredProducts = () => {
    return products.filter((product) => {
      ...
      return (
        categoryMatch &&
        subCategoryMatch &&
        (!showSearch || searchMatch || debouncedSearchTerm === "")
      );
    });
  };
  ...
  return (
    <ResponsiveContainer
      className={`border-t border-[#E5E7EB] flex flex-col justify-between ${showSearch ? `` : `pt-9`}`}
    >
      {showSearch && (
          ...
          <div className="flex items-center gap-2 sm:w-1/2">
            <div className="relative flex-grow">
              <input
                ...
                className="w-full px-4 py-2 pr-10 text-sm border border-[#9CA3AF] rounded-full focus:outline-none"
              />
              <img
                src={Search}
                alt="Search Icon"
                loading="lazy"
                className="w-4 absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none"
              />
            </div>
      ...
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="sm:w-[27.5%] sm:pr-10 pt-1">
          {/* Header Filter */}
          <button
            className="flex items-center w-full sm:block sm:text-left"
            onClick={() => setOpenFilters(!openFilters)}
          >
            <span className="text-xl">FILTERS</span>
            {/* Panah cuma muncul di mobile */}
            <span className="sm:hidden text-lg ml-2">
              {openFilters ? "▲" : "▼"}
            </span>
          </button>
        ...
        <div className="sm:w-3/4">
          <div className="flex justify-between text-2xl text-[#707070] mb-5 mt-5 sm:mt-0">
            ...
            <select
              ...
              className="border border-gray-300 text-sm px-1 md:px-3 md:pr-3 py-2 rounded shadow-sm focus:outline-none hover:cursor-pointer"
            >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {getSortedProducts().map((product) => (
              <ProductBox
                key={product._id}
                id={product._id}
                image={optimizeCloudinaryUrl(product.image[0])}
                name={product.name}
                price={product.price}
              />
            ))}
